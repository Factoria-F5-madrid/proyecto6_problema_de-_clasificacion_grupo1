# Archivo: src/preprocessing/vectorization.py
...
# Importamos la función de limpieza de texto del módulo P-04
# Asume que ya existe src/preprocessing/text_cleaning.py
...
TARGET_COLUMN = 'label_hate' # Nombre de la columna objetivo (0 o 1)

# Rutas de salida para guardar los resultados de la vectorización
...
def create_tfidf_features(df: pd.DataFrame, text_col: str, target_col: str):
    ...
    tfidf_vectorizer = TfidfVectorizer(
        min_df=1,             # Revertido: Usar 5 para ignorar ruido en el dataset real.
        max_df=0.9,           
        max_features=10000,   
...
if __name__ == '__main__':
    ...
    try:
        ...
        print(f"ADVERTENCIA: Archivo no encontrado en {DATA_PATH}. Usando datos de prueba balanceados.")
        
        # CORRECCIÓN: Usamos un dataset de 4 muestras (2 de cada clase) para que M-01 pueda entrenar.
        data = pd.DataFrame({
            TEXT_COLUMN: [
                "The hateful user posted a terrible link: https://hate-site.com!",
                "Get out of here you horrible troll, you are not welcome in this group.", 

                "We are running and jumping over fences, but we stopped running later.",
                "This is not hate speech, but a normal disagreement."
            ],
            TARGET_COLUMN: [1, 1, 0, 0] # Etiquetas actualizadas: dos 1s y dos 0s
        })
    ...
    # 3.1. Guardar la matriz de características X
    # Usamos .toarray() para guardar el sparse matrix como un array de numpy
    np.save(OUTPUT_FEATURES_PATH, X_features.toarray()) 
    
    # 3.2. Guardar el vector de etiquetas y
    np.save(OUTPUT_LABELS_PATH, y_labels.values)
    
    # 3.3. Guardar el Vectorizador entrenado (pickle)
